...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@urpi/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[urpiScrollbar]',
    exportAs: 'urpiScrollbar',
    standalone: true,
})
export class UrpiScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_urpiScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() urpiScrollbar: boolean = true;
    @Input() urpiScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('urpiScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.urpiScrollbar = coerceBooleanProperty(
                changes.urpiScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.urpiScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('urpiScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.urpiScrollbarOptions.currentValue
            );
