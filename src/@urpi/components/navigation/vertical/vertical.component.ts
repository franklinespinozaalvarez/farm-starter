...
import {
    UrpiNavigationItem,
    UrpiVerticalNavigationAppearance,
    UrpiVerticalNavigationMode,
    UrpiVerticalNavigationPosition,
} from '@urpi/components/navigation/navigation.types';
import { UrpiVerticalNavigationAsideItemComponent } from '@urpi/components/navigation/vertical/components/aside/aside.component';
...
import { UrpiScrollbarDirective } from '@urpi/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'urpi-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: urpiAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'urpiVerticalNavigation',
    standalone: true,
    imports: [
        UrpiScrollbarDirective,
        UrpiVerticalNavigationAsideItemComponent,
...
export class UrpiVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: UrpiVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: UrpiVerticalNavigationMode = 'side';
    @Input() name: string = this._urpiUtilsService.randomId();
    @Input() navigation: UrpiNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: UrpiVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<UrpiVerticalNavigationAppearance> =
        new EventEmitter<UrpiVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<UrpiVerticalNavigationMode> =
        new EventEmitter<UrpiVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<UrpiVerticalNavigationPosition> =
        new EventEmitter<UrpiVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<UrpiNavigationItem> =
        new ReplaySubject<UrpiNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<UrpiNavigationItem> =
        new ReplaySubject<UrpiNavigationItem>(1);
    ...
    private _urpiScrollbarDirectives!: QueryList<UrpiScrollbarDirective>;
    private _urpiScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        ...
        return {
            ...
            [`urpi-vertical-navigation-appearance-${this.appearance}`]: true,
            'urpi-vertical-navigation-hover': this._hovered,
            'urpi-vertical-navigation-inner': this.inner,
            'urpi-vertical-navigation-mode-over': this.mode === 'over',
            'urpi-vertical-navigation-mode-side': this.mode === 'side',
            'urpi-vertical-navigation-opened': this.opened,
            'urpi-vertical-navigation-position-left': this.position === 'left',
    ...
    /**
     * Setter for urpiScrollbarDirectives
     */
    @ViewChildren(UrpiScrollbarDirective)
    set urpiScrollbarDirectives(
        urpiScrollbarDirectives: QueryList<UrpiScrollbarDirective>
    ) {
        // Store the directives
        this._urpiScrollbarDirectives = urpiScrollbarDirectives;

        // Return if there are no directives
        if (urpiScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._urpiScrollbarDirectivesSubscription) {
            this._urpiScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._urpiScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                urpiScrollbarDirectives.forEach((urpiScrollbarDirective) => {
                    urpiScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._urpiScrollbarDirectives.forEach(
                    (urpiScrollbarDirective) => {
                        // Skip if not enabled
                        if (!urpiScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        urpiScrollbarDirective.scrollToElement(
                            '.urpi-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: UrpiNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: UrpiNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('urpi-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'urpi-vertical-navigation-overlay-transparent'
            );
        }
