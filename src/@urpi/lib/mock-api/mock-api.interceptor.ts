...
import { URPI_MOCK_API_DEFAULT_DELAY } from '@urpi/lib/mock-api/mock-api.constants';
import { UrpiMockApiService } from '@urpi/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(URPI_MOCK_API_DEFAULT_DELAY);
    const urpiMockApiService = inject(UrpiMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = urpiMockApiService.findHandler(
